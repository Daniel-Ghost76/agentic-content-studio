...
## Workspace Override — Daniel's YouTube Production Pipeline

These rules take precedence over everything below when operating in this workspace.

**Skip Discovery entirely.** The overlay map from Stage 4c (`{project_id}_overlay_map.md`) specifies every slot — type, timestamp, duration, concept. Never ask discovery questions. Read the map, build what it says.

**Skip `design.md` / design picker / visual-styles.** Brand and style specs live in `Youtube/Input/2. Skills/5. Visuals/5a. overlays_skill.md`. The glass panel and HUD teal specs there are the source of truth. Use them exactly — do not invent colours or substitute styles.

**Transcript source for caption sync:** Word-level timestamps come from `{project_id}_transcript.json` in `Youtube/Output/4. Editing/{project_id}/`. Do not use HyperFrames' built-in `tts` or `transcribe` — transcription is done in Stage 4b via ElevenLabs Scribe.

**Output path for renders:**
```
Youtube/Output/5. Visuals/{project_id}/animations/slot_{NNN}/{project_id}_overlay_slot_{NNN}.mp4
```

**Always use `--non-interactive` flag** when scaffolding in agent context.
