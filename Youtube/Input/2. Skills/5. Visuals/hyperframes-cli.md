...
npx hyperframes render . \
  -o "Youtube/Output/5. Visuals/{project_id}/animations/slot_{NNN}/{project_id}_overlay_slot_{NNN}.mp4"
```

**Quality guidance for this pipeline:**
- `--quality draft` — while building and iterating on a slot
- `--quality standard` — for the overlay_check review pass
- `--quality high --fps 30` — final compositing pass only

**Validation order before every render:** `lint` → `inspect` → render. Never skip inspect — it catches text overflow and container escapes that lint misses.
